...
@Singleton
public class ExpressionAction implements BotAction {
    ...
    public TeleResult action(Optional<TelegramObject> a) {
        ...
        teleResult.getRequests().add(
                new SendAnimation(
                        ...
                        gifService.getRandomGif("funny cats"))
        ...
        return telegramObject.filter(a -> {
            ...
            return
                    command != null && !command.startsWith("/")
                            ...
                            && containsAny(command, Arrays.asList(messagesServices.getMessage("uncensored.words").split(",")))
                    ;
