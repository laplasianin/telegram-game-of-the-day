...
@Singleton
public class BeerAction implements BotAction {
    ...
    public TeleResult action(Optional<TelegramObject> a) {
        ...
        teleResult.getRequests().add(
                new SendAnimation(
                        ...
                        gifService.getRandomGif("funny beer"))
    ...
    public boolean test(Optional<TelegramObject> telegramObject) {
        return telegramObject.filter(a -> {
            ...
            return
                    ...
                    && containsAny(command, Arrays.asList(messagesServices.getMessage("beer.words").split(",")))
                    ;
